...
from ..utils.async_error_utils import (
...
class BackgroundTaskProcessor:
    ...
    def __init__(self, db=None, _internal_init: bool = False):
        """Private initialiser - use BackgroundTaskProcessor.create() instead.
        ...
        if not _internal_init:
            raise RuntimeError(
                "BackgroundTaskProcessor must be created using the async factory method.\n"
                "Use: processor = await BackgroundTaskProcessor.create()\n"
                "Not: processor = BackgroundTaskProcessor()"
            )
    ...
    @classmethod
    async def create(cls, db=None):
        """Async factory method to create BackgroundTaskProcessor.

        Args:
            db: Optional database instance. If not provided, creates a new one.

        Returns:
            BackgroundTaskProcessor: Properly initialised processor instance
        """
        if db is None:
            db = await AgentDatabase.create()
        return cls(db=db, _internal_init=True)
    ...
    async def execute_task(self, task_data: Dict[str, Any]):
        ...
        try:
            ...
            logger.error(
                f"Task {task_id} ({function_name}) failed and marked as FAILED in database"
            )
...
async def start_background_processor():
    ...
    _processor_instance = await BackgroundTaskProcessor.create()
