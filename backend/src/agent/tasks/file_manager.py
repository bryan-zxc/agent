...
def generate_variable_path(
    planner_id: str, variable_key: str, check_existing: bool = False
) -> tuple[str, str]:
...
def generate_image_path(
    planner_id: str, image_key: str, check_existing: bool = False
) -> tuple[str, str]:
...
def save_execution_plan_model(
    planner_id: str, execution_plan_model: ExecutionPlanModel
) -> bool:
    ...
    try:
        ...
        logger.error(
            f"Failed to save execution plan model for planner {planner_id}: {e}"
        )
        return False
...
def load_execution_plan_model(planner_id: str) -> Optional[ExecutionPlanModel]:
    ...
    try:
        ...
        logger.error(
            f"Failed to load execution plan model for planner {planner_id}: {e}"
        )
        return None
...
def save_worker_message_history(
    planner_id: str, task_responses: list[TaskResponseModel]
) -> bool:
    ...
    try:
        ...
        logger.error(
            f"Failed to save worker message history for planner {planner_id}: {e}"
        )
        return False
...
def load_worker_message_history(planner_id: str) -> list[TaskResponseModel]:
    ...
    try:
        ...
        if not file_path.exists():
            logger.debug(
                f"Worker message history file not found: {file_path}, returning empty list"
            )
            return []
        ...
        logger.error(
            f"Failed to load worker message history for planner {planner_id}: {e}"
        )
        return []


def append_to_worker_message_history(
    planner_id: str, task_response: TaskResponseModel
) -> bool:
    ...
    try:
        ...
        logger.error(
            f"Failed to append to worker message history for planner {planner_id}: {e}"
        )
        return False
...
def save_wip_answer_template(planner_id: str, template_content: str) -> bool:
    ...
    try:
        ...
        logger.error(
            f"Failed to save WIP answer template for planner {planner_id}: {e}"
        )
        return False
...
def load_wip_answer_template(planner_id: str) -> Optional[str]:
    ...
    try:
        ...
        logger.error(
            f"Failed to load WIP answer template for planner {planner_id}: {e}"
        )
        return None
