...
class AsyncErrorLogger:
    ...
    def log_exception_group(
        self, exc: BaseExceptionGroup, context: Optional[str] = None
    ) -> None:
        """
        ...
        logger.error(
            f"ExceptionGroup occurred{context_msg}: {len(exc.exceptions)} sub-exception(s)"
        )
    ...
    def log_task_exception(
        self, task: asyncio.Task, task_name: Optional[str] = None
    ) -> None:
        """
...
def log_taskgroup_errors(
    exc: BaseException, context: str = "TaskGroup operation"
) -> None:
    """
...
async def run_with_detailed_logging(
    coro_func, *args, context: str = "async_function", **kwargs
) -> Any:
    """
...
def create_task_with_logging(
    coro, name: Optional[str] = None, context: str = "background_task"
) -> asyncio.Task:
    """
