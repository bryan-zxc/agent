...
def initial_plan_to_execution_plan_model(
    initial_plan: InitialExecutionPlan,
) -> ExecutionPlanModel:
...
def execution_plan_model_to_markdown(model: ExecutionPlanModel) -> str:
    ...
    markdown_lines = ["# Objective", model.objective, "", "# Todos"]

    for todo in model.todos:
        # Use updated_description if available, otherwise use original description
        description = (
            todo.updated_description if todo.updated_description else todo.description
        )

        if todo.completed:
            markdown_lines.append(f"- [x] ~~{description}~~")
        elif todo.obsolete:
            markdown_lines.append(f"- [x] ~~OBSOLETE: {description}~~")
        else:
...
def validate_execution_plan_model(
    model: ExecutionPlanModel, previous_model: ExecutionPlanModel = None
) -> Tuple[bool, List[str]]:
    """
    ...
    if previous_model:
        if len(model.todos) < len(previous_model.todos):
            errors.append(
                f"Task list shrunk from {len(previous_model.todos)} to {len(model.todos)} - tasks cannot be removed"
            )
