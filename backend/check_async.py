...
def main():
    ...
    parser.add_argument("--module", help="Check specific module (tasks, core, models, services, utils)")
    ...
    if not args.mypy_only:
        if isinstance(target_path, list):
            for path in target_path:
                ...
                ], f"Ruff async checks on {path}", show_output=not args.quiet))
        else:
            checks.append(run_command([
                "uv", "run", "ruff", "check", target_path, "--select", "ASYNC"
            ], f"Ruff async checks on {target_path}", show_output=not args.quiet))
        ...
        if isinstance(target_path, list):
            ...
            for file_path in critical_files:
                if Path(file_path).exists():
                    checks.append(run_command([
                        "uv", "run", "mypy", file_path, "--config-file", "mypy.ini", "--show-error-codes"
                    ], f"MyPy coroutine checks on {file_path}", show_output=not args.quiet))
        else:
            checks.append(run_command([
                "uv", "run", "mypy", target_path, "--config-file", "mypy.ini", "--show-error-codes"
            ], f"MyPy coroutine checks on {target_path}", show_output=not args.quiet))
