...
class TestMessageManager(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_add_message_returns_updated_list(self):
        ...
        with patch.object(
            self.db, "add_message", new_callable=AsyncMock
        ) as mock_add, patch.object(
            self.db, "get_messages", new_callable=AsyncMock
        ) as mock_get:
    ...
    async def test_clear_messages_removes_all(self):
        ...
        with patch.object(
            self.db, "clear_messages", new_callable=AsyncMock
        ) as mock_clear:
            message_manager = MessageManager(self.db, self.agent_type, self.agent_id)
    ...
    async def test_concurrent_message_operations(self):
        ...
        with patch.object(
            ...
            results = await asyncio.gather(*[add_message(i) for i in range(3)])
