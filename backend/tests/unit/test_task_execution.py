...
from src.agent.models.tasks import (
    TaskResult,
    ...
    async def test_update_planner_next_task_and_queue_async_correctness(self):
        ...
        try:
            async with self.capture_async_warnings() as warning_list:
                ...
                # Mock the AgentDatabase.create factory method to return our test database
                with patch("src.agent.tasks.task_utils.AgentDatabase") as mock_db_class:
                    mock_db_class.create = AsyncMock(return_value=db)
...
class TestPlannerTasksExecution(
    ...
    def test_get_table_metadata_vanilla_execution(self):
        ...
        try:
            ...
            self.assertEqual(
                ...
                "<class 'src.agent.models.schemas.TableMeta'>",
            )
            ...
            single_val_col = SingleValueColumn.model_validate(
                {"column_name": "test_column", "only_value_in_column": "test_value"}
            )
    ...
    async def test_execute_initial_planning_vanilla_execution(self):
        ...
        async with self.capture_async_warnings() as warning_list:
            ...
            with patch("src.agent.tasks.planner_tasks.AgentDatabase") as MockDB, patch(
                "src.agent.tasks.planner_tasks.llm"
            ) as mock_llm_service, patch(
                "src.agent.tasks.planner_tasks.save_execution_plan_model"
            ) as mock_save_plan, patch(
                "src.agent.tasks.planner_tasks.save_answer_template"
            ) as mock_save_template, patch(
                "src.agent.tasks.planner_tasks.save_wip_answer_template"
            ) as mock_save_wip, patch(
                "src.agent.tasks.planner_tasks.update_planner_next_task_and_queue"
            ) as mock_queue, patch(
                "src.agent.tasks.planner_tasks.MessageManager"
            ) as MockMM:
                ...
                async def mock_llm_responses(*args, **kwargs):
                    ...
                    if not hasattr(mock_llm_responses, "call_count"):
                        mock_llm_responses.call_count = 0
                    mock_llm_responses.call_count += 1

                    if mock_llm_responses.call_count == 1:
                        return InitialExecutionPlan(
                            objective="Test objective", todos=["task1", "task2"]
                        )
                    else:
                        return type(
                            "MockResponse", (), {"content": "# Test Answer Template"}
                        )()

                mock_llm_service.a_get_response = mock_llm_responses
        ...
        async_warnings = [
            ...
            if "unawaited coroutine" in str(w.message)
            or "was never awaited" in str(w.message)
        ]
        if async_warnings:
            self.fail(
                f"Async warnings detected: {[str(w.message) for w in async_warnings]}"
            )
    ...
    async def test_execute_task_creation_vanilla_execution(self):
        ...
        ) as mock_load_plan, patch(
            "src.agent.tasks.planner_tasks.save_current_task"
        ) as mock_save_task, patch(
            "src.agent.tasks.planner_tasks.load_worker_message_history"
        ) as mock_load_history, patch(
            "src.agent.tasks.planner_tasks.queue_worker_task"
        ) as mock_queue_worker, patch(
            "src.agent.tasks.planner_tasks.MessageManager"
        ) as MockMM, patch(
            "src.agent.tasks.planner_tasks.has_pending_tasks"
        ) as mock_has_pending, patch(
            "src.agent.tasks.planner_tasks.get_next_action_task"
        ) as mock_get_next:
    ...
    async def test_complete_planner_execution_vanilla_execution(self):
        ...
        with patch(
            "src.agent.tasks.planner_tasks.load_wip_answer_template"
        ) as mock_load_template, patch(
            "src.agent.tasks.planner_tasks.llm"
        ) as mock_llm_service, patch(
            "src.agent.tasks.planner_tasks.save_execution_plan_model"
        ) as mock_save_plan, patch(
            "src.agent.tasks.planner_tasks.execution_plan_model_to_markdown"
        ) as mock_plan_to_md:
            ...
            execution_plan_model = ExecutionPlanModel(
                objective="Test objective", todos=[]
            )
            planner_data = {"model": "gpt-4", "temperature": 0.0}
...
class TestWorkerTasksExecution(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_validate_worker_result_vanilla_execution(self):
        ...
        with patch(
            "src.agent.services.llm_service.LLM.a_get_response", autospec=True
        ) as mock_llm:
    ...
    async def test_process_image_variable_vanilla_execution(self):
        ...
        with patch(
            "src.agent.utils.tools.encode_image", return_value="encoded_image"
        ), patch(
            "src.agent.tasks.file_manager.generate_image_path",
            return_value=("/path/image.png", "test_image"),
        ), patch(
            "src.agent.tasks.file_manager.save_image_to_file", return_value=True
        ):
    ...
    async def test_process_variable_vanilla_execution(self):
        ...
        with patch(
            "src.agent.tasks.file_manager.generate_variable_path",
            return_value=("/path/var.pkl", "test_variable"),
        ), patch(
            "src.agent.tasks.file_manager.save_variable_to_file", return_value=True
        ), patch(
            "src.agent.utils.tools.is_serialisable", return_value=(True, True)
        ):
    ...
    async def test_worker_initialisation_vanilla_execution(self):
        ...
        with patch("src.agent.tasks.worker_tasks.AgentDatabase") as MockDB, patch(
            "src.agent.tasks.worker_tasks.update_worker_next_task_and_queue"
