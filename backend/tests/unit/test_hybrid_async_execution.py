...
class HybridAsyncTestCase(unittest.IsolatedAsyncioTestCase, AsyncWarningCaptureMixin):
    ...
    async def asyncTearDown(self):
        ...
        if hasattr(self.db, "async_engine"):
    ...
    async def hybrid_mock_context(self):
        ...
        """
        with patch("src.agent.tasks.planner_tasks.llm") as mock_llm, patch(
            ...
            "src.agent.tasks.planner_tasks.AgentDatabase"
        ) as mock_planner_db_class, patch(
            "src.agent.tasks.task_utils.AgentDatabase"
        ) as mock_utils_db_class:

            # Configure database mocks to return our real test database
            mock_planner_db_class.create = AsyncMock(return_value=self.db)
            mock_utils_db_class.create = AsyncMock(return_value=self.db)
...
class TestHybridPlannerAsyncExecution(HybridAsyncTestCase):
    ...
    async def test_execute_initial_planning_real_async_flow(self):
        ...
        # Create router and message first (needed for foreign key constraints)
        ...
        task_data = {
            ...
            "payload": {
                ...
                "message_id": actual_message_id,  # Use the actual message ID from database
        ...
        async with self.capture_async_warnings() as warnings_list:
            async with self.hybrid_mock_context() as mocks:
                ...
                performance = await self.measure_async_performance(
                    execute_initial_planning, task_data
                )
                ...
                self.assertEqual(
                    planner_data["status"], "executing"
                )  # Status updated after successful plan creation
                ...
                self.performance_metrics["execute_initial_planning"] = performance[
                    "execution_time"
                ]
    ...
    async def test_concurrent_database_operations_async_flow(self):
        ...
        async with self.capture_async_warnings() as warnings_list:
            ...
            with patch("src.agent.tasks.task_utils.AgentDatabase") as mock_db_class:
                mock_db_class.create = AsyncMock(return_value=self.db)
                ...
                for result in results:
                    ...
                    self.assertTrue(
                        result["result"]
                    )  # update_planner_next_task_and_queue returns bool
...
class TestHybridWorkerAsyncExecution(HybridAsyncTestCase):
    ...
    async def test_queue_worker_task_real_database_operations(self):
        ...
        async with self.capture_async_warnings() as warnings_list:
            ...
            with patch("src.agent.tasks.task_utils.AgentDatabase") as mock_db_class:
                ...
                worker_tasks = [
                    t for t in tasks if t.get("entity_id") == self.worker_id
                ]
                self.assertTrue(len(worker_tasks) > 0)

                # Store performance metrics
                self.performance_metrics["queue_worker_task"] = performance[
                    "execution_time"
                ]
    ...
    async def test_execute_standard_worker_with_mocked_externals(self):
        ...
        async with self.capture_async_warnings() as warnings_list:
            ...
            with patch("src.agent.tasks.worker_tasks.llm") as mock_worker_llm, patch(
                "src.agent.tasks.worker_tasks.load_current_task"
            ) as mock_load_task, patch(
                "src.agent.tasks.task_utils.AgentDatabase"
            ) as mock_db_class:

                # Configure mocks
                mock_db_class.create = AsyncMock(return_value=self.db)
                mock_load_task.return_value = type(
                    "MockTask",
                    (),
                    {
                        "task_id": "test_task",
                ...
                mock_worker_llm.a_get_response.return_value = type(
                    "MockResponse", (), {"content": "Test worker analysis complete"}
                )()

                # Execute with performance measurement
                performance = await self.measure_async_performance(
                    execute_standard_worker, task_data
                )
                ...
                # Store performance metrics
                self.performance_metrics["execute_standard_worker"] = performance[
                    "execution_time"
                ]
    ...
    async def test_async_execution_performance_benchmarks(self):
        ...
        async with self.capture_async_warnings() as warnings_list:
            ...
            with patch("src.agent.tasks.task_utils.AgentDatabase") as mock_db_class:
                ...
                multi_db_perf = await self.measure_async_performance(
                    multi_db_operations
                )
                ...
                for func_name, actual_time in performance_results.items():
                    ...
                    logger.info(
                        f"  {status} {func_name}: {actual_time:.4f}s (target: {target_time:.2f}s)"
                    )
    ...
    async def test_concurrent_async_execution_overhead(self):
        ...
        async with self.capture_async_warnings() as warnings_list:
            with patch("src.agent.tasks.task_utils.AgentDatabase") as mock_db_class:
                ...
                ):  # Only fail if extremely slower
                    logger.warning(
                        f"Concurrent execution much slower than expected: {concurrent_time:.4f}s vs {sequential_time:.4f}s"
                    )

                # Main assertion: ensure concurrent execution completed successfully
                self.assertGreater(
                    concurrency_improvement, 0, "Concurrency measurement failed"
                )
                ...
                self.performance_metrics["concurrency_improvement"] = (
                    concurrency_improvement
                )
