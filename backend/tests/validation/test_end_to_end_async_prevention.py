...
class EndToEndAsyncPreventionTestCase(unittest.IsolatedAsyncioTestCase, AsyncWarningCaptureMixin):
...
'''
    ...
    async def measure_validation_layer_performance(self, layer_name: str, validation_func) -> Dict[str, Any]:
        """Measure performance of a specific validation layer."""
        start_time = time.time()
        
        try:
            result = await validation_func() if asyncio.iscoroutinefunction(validation_func) else validation_func()
            success = True
            error = None
...
class TestStaticAnalysisValidation(EndToEndAsyncPreventionTestCase):
    ...
    async def test_ruff_catches_async_antipatterns(self):
        ...
        self.assertLess(performance['execution_time'], 5.0, 
        ...
        if not result['issues_found']:
            ...
            self.assertTrue(len(output_text) > 0,
                           "Should have some output when issues are detected")
    ...
    async def test_runtime_warning_capture_effectiveness(self):
        ...
        async def runtime_warning_validation():
            ...
            return {
                ...
                'unawaited_coroutines_found': any('coroutine' in str(w.message).lower() 
        ...
        self.assertLess(performance['execution_time'], 1.0,
                       f"Runtime testing took {performance['execution_time']:.2f}s (target: <1s)")
        
        # Verify warning capture
        result = performance['result']
        self.assertGreater(result['warnings_captured'], 0,
                          "Should capture at least one async warning")
        self.assertTrue(result['unawaited_coroutines_found'],
    ...
    async def test_async_warning_test_decorator(self):
        ...
        async def decorator_validation():
            ...
            return {
                ...
                'warnings_caught': len(unawaited_warnings) >= 2,  # Expect at least 2 warnings
        ...
        result = performance['result']
        self.assertTrue(result['decorator_worked'],
...
class TestIntegrationTestingValidation(EndToEndAsyncPreventionTestCase):
    ...
    async def test_integration_test_async_coverage(self):
        ...
        self.assertTrue(performance['success'], "Integration tests should run successfully")
        self.assertLess(performance['execution_time'], 30.0,
                       f"Integration testing took {performance['execution_time']:.2f}s (target: <30s)")
        
        result = performance['result']
        self.assertEqual(result['tests_completed'], 3,
                        "Should complete all integration test scenarios")
        self.assertTrue(result['system_level_validation'],
...
class TestEndToEndValidationEffectiveness(EndToEndAsyncPreventionTestCase):
    ...
    async def test_original_async_bug_prevention(self):
        ...
        async def comprehensive_validation():
            ...
            try:
                static_result = subprocess.run([
                    sys.executable, '-m', 'ruff', 'check',
                    str(original_bug_file), '--select', 'ASYNC'
                ], capture_output=True, text=True)
                
                if static_result.returncode != 0:
                    validation_results['static_analysis_caught'] = True
                    validation_results['layers_that_caught_bug'].append('static_analysis')
            except Exception:
        ...
        self.assertLess(performance['execution_time'], 60.0,
        ...
        self.assertGreaterEqual(layers_caught, 2,
        ...
        print(f"   Original bug caught by {layers_caught} layers: {result['layers_that_caught_bug']}")
    ...
    async def test_multi_layer_performance_targets(self):
        ...
        async def performance_validation():
            ...
            return {
                'static_analysis_time': self.layer_performance.get('static_analysis', 0),
                'runtime_testing_time': self.layer_performance.get('runtime_testing', 0),
                'integration_testing_time': self.layer_performance.get('integration_testing', 0),
                'end_to_end_time': self.layer_performance.get('end_to_end_validation', 0),
    ...
    async def test_validation_coverage_completeness(self):
        ...
        async def coverage_validation():
            ...
            issue_types = ['missing_await', 'unawaited_coroutine', 'blocking_call_in_async', 'race_condition']
            coverage_results = {}
            ...
            return {
                ...
                'static_analysis_coverage': sum(1 for r in coverage_results.values() 
                                               if r.get('static_analysis', False)),
                'total_testable_issues': sum(1 for r in coverage_results.values() 
                                            if r.get('testable', False))
            }
        ...
        self.assertEqual(result['issue_types_tested'], 4,
                        "Should test all 4 major async issue types")
        self.assertEqual(result['total_testable_issues'], 4,
                        "All async issue types should be testable")
        self.assertGreaterEqual(result['static_analysis_coverage'], 2,
        ...
        for issue_type, coverage in result['coverage_results'].items():
            ...
            print(f"   {status} {issue_type}: Static={coverage.get('static_analysis', False)}")
