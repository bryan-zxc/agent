...
class MockWebSocketManager:
    ...
    def get_messages_for_router(self, router_id: str) -> List[WebSocketMessage]:
        ...
        return [msg for msg in self.message_history if msg.router_id == router_id]
    ...
    async def send_json(self, data: Dict[str, Any]):
        ...
        if self.connection_state != "connected":
            raise ConnectionError(
                f"WebSocket connection {self.connection_id} not connected"
            )
...
class WebSocketAsyncCommunicationTestCase(
    ...
    async def measure_websocket_performance(
        ...
        try:
            ...
            message_throughput = result.get("messages_processed", 0) / max(
            ...
            return {
                ...
                "performance_data": {
                    ...
                    <= self.websocket_performance_targets.get(test_name, 15.0),
        ...
        async with self.capture_async_warnings() as warnings_list:
            async with self.websocket_mock_context() as mocks:
                ...
                performance = await self.measure_websocket_performance(
                    "real_time_updates", self._execute_router_websocket_coordination
                )
    ...
    async def test_concurrent_websocket_message_delivery(self):
        ...
        async with self.capture_async_warnings() as warnings_list:
            async with self.websocket_mock_context() as mocks:
                ...
                performance = await self.measure_websocket_performance(
                    "message_delivery", self._execute_concurrent_message_delivery
                )
                ...
                self.assertGreater(
                    result["delivery_success_rate"], 0.95
                )  # 95% success rate
    ...
    async def test_connection_stability_during_async_operations(self):
        ...
        async with self.capture_async_warnings() as warnings_list:
            async with self.websocket_mock_context() as mocks:
                ...
                performance = await self.measure_websocket_performance(
                    "connection_management", self._execute_connection_stability_test
                )
        ...
        async def long_running_operation():
            ...
            for i in range(10):
                ...
                await self.db.update_planner(
                    self.planner_id, current_task=f"stability_test_step_{i}"
                )
