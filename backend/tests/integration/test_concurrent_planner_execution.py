...
class TestConcurrentPlannerExecution(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_concurrent_task_queue_isolation(self):
        ...
        for i, planner_id in enumerate(planner_ids):
            ...
            planner_tasks = [
                task for task in all_pending if task["entity_id"] == planner_id
            ]
            self.assertEqual(
                len(planner_tasks), 5, f"Planner {planner_id} should have 5 tasks"
            )
    ...
    async def test_concurrent_database_consistency(self):
        ...
        def update_planner_data(planner_id, updates):
            ...
            for i, update_data in enumerate(updates):
                ...
                results.append(
                    f"update_{i}_success" if success else f"update_{i}_failed"
                )
        ...
        for i, planner_id in enumerate(planner_ids):
            ...
            self.assertEqual(
                ...
                3,
                f"Planner {planner_id} should have 3 assistant messages",
            )
    ...
    async def test_task_queue_ordering_under_concurrency(self):
        ...
        planner_tasks = [
            task for task in queued_tasks if task["entity_id"] == planner_id
        ]
    ...
    async def test_concurrent_worker_task_creation(self):
        ...
        for planner_id in planner_ids:
            ...
            self.assertEqual(
                len(workers), 4, f"Planner {planner_id} should have 4 workers"
            )
