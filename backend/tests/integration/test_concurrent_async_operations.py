...
class ConcurrentAsyncOperationsTestCase(unittest.IsolatedAsyncioTestCase, AsyncWarningCaptureMixin):
    ...
    async def measure_concurrent_performance(self, test_name, async_func, *args, **kwargs):
        ...
        try:
            ...
            operations_per_second = result.get('total_operations', 0) / max(execution_time, 0.001)
            ...
            return {
                ...
                'performance_data': {
                    ...
                    'within_target': execution_time <= self.concurrent_performance_targets.get(test_name, 60.0),
...
class TestConcurrentPlannerExecution(ConcurrentAsyncOperationsTestCase):
        ...
        async with self.capture_async_warnings() as warnings_list:
            async with self.concurrent_mock_context() as mocks:
                ...
                performance = await self.measure_concurrent_performance(
                    'concurrent_planners',
                    self._execute_concurrent_planners
                )
                ...
                self.assertEqual(result['successful_planners'], 
                ...
                self.assertFalse(result['race_conditions_detected'], 
    ...
    async def _execute_concurrent_planners(self):
        ...
        planner_results = await asyncio.gather(*concurrent_tasks, return_exceptions=True)
        
        # Analyse concurrent execution results
        successful_planners = sum(
            1 for result in planner_results 
        ...
        async with self.capture_async_warnings() as warnings_list:
            async with self.concurrent_mock_context() as mocks:
                ...
                performance = await self.measure_concurrent_performance(
                    ...
                    self._execute_database_stress_test
                )
                ...
                result = performance['result']
                self.assertGreater(result['successful_operations'], 
    ...
    async def _single_database_stress_operation(self, operation_id, index):
        ...
        try:
            ...
            await self.db.update_planner(
                planner_id,
                status="executing",
                current_task=f"stress_task_{index}"
            )
...
class TestResourceContentionPrevention(ConcurrentAsyncOperationsTestCase):
    ...
    async def test_resource_contention_detection_and_prevention(self):
        ...
        async with self.capture_async_warnings() as warnings_list:
            async with self.concurrent_mock_context() as mocks:
                ...
                performance = await self.measure_concurrent_performance(
                    'resource_contention',
                    self._execute_resource_contention_test
                )
                ...
                result = performance['result']
                self.assertTrue(result['contention_handled'],
    ...
    async def _execute_resource_contention_test(self):
        ...
        successful_contentions = sum(
            1 for result in contention_results 
            if not isinstance(result, Exception) and result.get('success', False)
        )
