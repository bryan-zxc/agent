...
class TestFastAPIImmediateResponse(unittest.IsolatedAsyncioTestCase):
    ...
    def test_chat_endpoint_immediate_response(self):
        ...
        with TestClient(app) as client:
            ...
            response = client.post(
                "/api/chat",
                json={"message": "Test message", "router_id": "test_router"},
            )
            ...
            self.assertLess(
                response_time,
                0.1,
                f"Response took {response_time:.3f}s, should be under 0.1s",
            )
    ...
    def test_upload_endpoint_immediate_response(self):
        ...
        with TestClient(app) as client:
            ...
            response = client.post(
                "/api/upload",
                json={"files": ["file1.csv", "file2.png"], "router_id": "test_router"},
            )
            ...
            self.assertLess(
                response_time,
                0.1,
                f"Upload response took {response_time:.3f}s, should be under 0.1s",
            )
    ...
    def test_status_endpoint_immediate_response(self):
        ...
        with TestClient(app) as client:
            ...
            self.assertLess(
                response_time,
                0.1,
                f"Status response took {response_time:.3f}s, should be under 0.1s",
            )
    ...
    def test_concurrent_requests_immediate_response(self):
        ...
        def make_concurrent_request(client, endpoint, data=None):
            ...
            if endpoint == "chat":
                ...
                response = client.post(
                    "/api/upload", json=data or {"files": ["test.csv"]}
                )
            elif endpoint == "status":
                response = client.get(
                    f"/api/conversations/{data or 'test_router'}/status"
                )
        ...
        with TestClient(app) as client:
            ...
            for i in range(10):
                if i % 3 == 0:
                    ...
                    thread = threading.Thread(
                        target=lambda i=i: results.append(
                            make_concurrent_request(
                                client, "upload", {"files": [f"test{i}.csv"]}
                            )
                        )
                    )
                else:
            ...
            for response_time, status_code in results:
                ...
                self.assertLess(
                    response_time, 0.1, f"Concurrent request took {response_time:.3f}s"
                )

            # Assert total time is reasonable (should be close to single request time due to concurrency)
            self.assertLess(
                total_time, 0.5, f"All concurrent requests took {total_time:.3f}s"
            )

    @patch("agent.tasks.task_utils.update_planner_next_task_and_queue")
    @patch("agent.models.agent_database.AgentDatabase")
    def test_background_task_queueing_does_not_delay_response(
        self, mock_db_class, mock_queue_task
    ):
        ...
        mock_db.create_planner.side_effect = lambda *args, **kwargs: time.sleep(
            0.05
        )  # 50ms delay

        # Mock task queueing to take some time
        mock_queue_task.side_effect = lambda *args, **kwargs: time.sleep(
            0.03
        )  # 30ms delay
        ...
        with TestClient(app) as client:
            ...
            response = client.post(
                "/api/process",
                json={"task": "complex processing task", "data": "large dataset"},
            )
            ...
            self.assertLess(
                response_time,
                0.1,
                f"Response with background queueing took {response_time:.3f}s",
            )
            self.assertEqual(response.status_code, 200)
    ...
    def test_response_time_consistency(self):
        ...
        with TestClient(app) as client:
            ...
            for i in range(20):
                ...
                response = client.post(
                    "/api/chat",
                    json={"message": f"Test message {i}", "router_id": f"router_{i}"},
                )
        ...
        variance = sum((t - avg_response_time) ** 2 for t in response_times) / len(
            response_times
        )
        std_dev = variance**0.5

        self.assertLess(
            std_dev, 0.02, f"Response time standard deviation {std_dev:.3f}s too high"
        )

    def test_router_websocket_immediate_connection(self):
        ...
        async def websocket_endpoint(websocket, router_id: str):
            ...
            try:
                while True:
                    ...
                    await websocket.send_json(
                        {"type": "echo", "data": data, "timestamp": time.time()}
                    )
            except:
                pass
        ...
        # Test passes if no errors occur in setting up the WebSocket endpoint
        self.assertTrue(True, "WebSocket endpoint setup successful")
    ...
    def test_error_responses_immediate(self):
        ...
        with TestClient(app) as client:
            ...
            self.assertLess(
                success_time, 0.1, f"Success response took {success_time:.3f}s"
            )
            ...
            self.assertLess(
                time_difference,
                0.05,
                "Error and success response times should be similar",
            )
    ...
    def test_response_time_statistics(self):
        ...
        if self.response_times:
            ...
            self.assertLess(
                avg_time, 0.1, "Average response time should be under 100ms"
            )
            self.assertLess(
                max_time, 0.2, "Maximum response time should be under 200ms"
            )
