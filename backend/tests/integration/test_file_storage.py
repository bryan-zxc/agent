...
class TestFileStorage(unittest.IsolatedAsyncioTestCase):
    ...
    def test_generate_variable_path_basic(self):
        ...
        expected_path = str(
            Path(self.test_dir) / self.planner_id / "variables" / "test_var.pkl"
        )
    ...
    def test_generate_variable_path_with_collision(self):
        ...
        file_path, final_key = generate_variable_path(
            self.planner_id, "test_var", check_existing=True
        )
    ...
    def test_generate_image_path_basic(self):
        ...
        expected_path = str(
            Path(self.test_dir) / self.planner_id / "images" / "test_img.b64"
        )
        self.assertEqual(file_path, expected_path)
    ...
    def test_generate_image_path_with_collision(self):
        ...
        file_path, final_key = generate_image_path(
            self.planner_id, "test_img", check_existing=True
        )
    ...
    async def test_save_planner_variable(self, mock_db_class):
        ...
        # Mock database
        mock_db = AsyncMock()
        mock_db_class.create = AsyncMock(return_value=mock_db)
        mock_db.get_planner.return_value = {"variable_file_paths": {}}

        # Test save
        file_path, final_key = await save_planner_variable(
            self.planner_id, "test_var", self.test_variable_data
        )
        ...
        self.assertIn(
            "variable_file_paths", call_args[1]
        )  # variable_file_paths in kwargs
    ...
    async def test_save_planner_image(self, mock_db_class):
        ...
        # Test save
        file_path, final_key = await save_planner_image(
            self.planner_id, "test_img", self.test_image_data
        )
    ...
    def test_clean_image_name(self):
        ...
        # Test basic cleaning
        self.assertEqual(
            clean_image_name("test-image.jpg", existing_names), "test_image_jpg"
        )

        # Test special characters
        self.assertEqual(
            clean_image_name("test@#$%image", existing_names), "test_image"
        )
        ...
        self.assertEqual(
            clean_image_name("existing_image", existing_names), "existing_image_1"
        )
    ...
    async def test_collision_avoidance_variables(self, mock_db_class):
        ...
        # Save first variable
        file_path1, key1 = await save_planner_variable(
            self.planner_id, "test_var", {"data": 1}
        )
        self.assertEqual(key1, "test_var")

        # Save second variable with same name but collision checking
        file_path2, key2 = await save_planner_variable(
            self.planner_id, "test_var", {"data": 2}, check_existing=True
        )
    ...
    async def test_collision_avoidance_images(self, mock_db_class):
        ...
        # Save first image
        file_path1, key1 = await save_planner_image(
            self.planner_id, "test_img", "image_data_1"
        )
        self.assertEqual(key1, "test_img")

        # Save second image with same name but collision checking
        file_path2, key2 = await save_planner_image(
            self.planner_id, "test_img", "image_data_2", check_existing=True
        )
