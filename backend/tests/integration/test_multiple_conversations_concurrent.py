...
class TestMultipleConversationsConcurrent(unittest.IsolatedAsyncioTestCase):
    ...
    async def asyncSetUp(self):
        ...
        self.test_dir = tempfile.mkdtemp(prefix="test_concurrent_")
        ...
        self.test_conversations = [
            {
                ...
                "messages": [
                    ...
                    {
                        "role": "assistant",
                        "content": "I'll help you analyse the Q1 sales data. Let me start by examining the dataset.",
                    },
                    {
                        "role": "user",
                        "content": "Focus on regional performance differences",
                    },
                ],
            },
            {
                ...
                "messages": [
                    {
                        "role": "user",
                        "content": "Process these product images for the catalogue",
                    },
                    {
                        "role": "assistant",
                        "content": "I'll process the product images for your catalogue. Starting with image enhancement.",
                    },
            ...
            },
            {
                ...
                "messages": [
                    {
                        "role": "user",
                        "content": "Research the latest market trends in renewable energy",
                    },
                    {
                        "role": "assistant",
                        "content": "I'll research the current renewable energy market trends for you.",
                    },
                    {
                        "role": "user",
                        "content": "Include solar and wind energy specifically",
                    },
                ],
            },
        ]
    ...
    async def create_test_router(self, conversation_data: dict) -> str:
        ...
        await self.db.create_router(
            ...
            preview=(
                conversation_data["messages"][0]["content"][:100]
    ...
    async def test_concurrent_conversation_creation(self):
        ...
        async def create_conversation(conversation_data, index):
            ...
            conversation_data = {
                **conversation_data,
                "title": f"{conversation_data['title']} #{index}",
            }
        ...
        for router_id in created_routers:
            ...
            self.assertGreaterEqual(
                len(messages), 3
            )  # At least 3 messages per conversation

    async def test_concurrent_message_addition(self):
        """Test concurrent message addition to different conversations."""
        # Create test routers
        router_ids = [
            await self.create_test_router(conv) for conv in self.test_conversations
        ]
        ...
        message_batches = [
            ...
            [
                ...
                "Contrast enhanced successfully",
                "Apply sharpening filter",
            ],
            [
                "Latest solar panel efficiency?",
                "Efficiency reached 22% average",
                "Compare with wind power",
            ],
        ]
        ...
        for i, router_id in enumerate(router_ids):
            ...
            # Messages may be combined if consecutive roles are the same
            # So check for minimum expected (accounting for combining)
            min_expected = 4  # Worst case: all same-role messages combined
            max_expected = len(self.test_conversations[i]["messages"]) + len(
                message_batches[i]
            )
            self.assertGreaterEqual(len(messages), min_expected)
            self.assertLessEqual(len(messages), max_expected)

            # Verify message content contains router-specific identifier
            router_suffix = router_id[-8:]

            # Check that router-specific content exists in messages
            all_content = []
            for msg in messages:
            ...
            # Verify router suffix appears in the new messages
            router_content = [c for c in all_content if router_suffix in c]
            self.assertEqual(len(router_content), len(message_batches[i]))
    ...
    async def test_concurrent_conversation_file_operations(self):
        ...
        def perform_file_operations(router_id, file_prefix):
            ...
            for i in range(3):
                ...
                file_content = (
                    f"Content for {file_prefix}_{i} in conversation {router_id[-8:]}"
                )
    ...
    async def test_concurrent_conversation_state_updates(self):
        ...
        async def update_conversation_state(router_id, status_sequence):
            ...
            for i, status in enumerate(status_sequence):
                ...
                new_title = (
                    f"Conversation {router_id[-8:]} - {status.title()} (Step {i+1})"
                )
    ...
    async def test_conversation_isolation_under_load(self):
        ...
        for i in range(num_conversations):
            conv_data = {
                ...
                "messages": [
                    ...
                    {
                        "role": "assistant",
                        "content": f"Starting processing for batch {i}",
                    },
                ],
            }
        ...
        for i, router_id in enumerate(router_ids):
            ...
            messages = await self.db.get_messages("router", router_id)
            expected_messages = (
                2 + operations_per_conversation
            )  # Original + load test messages
            self.assertEqual(len(messages), expected_messages)

            # Verify messages contain router-specific content
            router_suffix = router_id[-8:]
            load_test_messages = [
                msg for msg in messages if "Load test" in msg["content"]
            ]
