...
class TestBackgroundProcessorEfficiency(unittest.IsolatedAsyncioTestCase):
    ...
    async def create_mock_processor(self):
        """Create a mock background processor for testing."""
        # Create processor with our test database
        processor = await BackgroundTaskProcessor.create(db=self.db)
    ...
    async def queue_test_tasks(self, task_configs):
        ...
        for config in task_configs:
            ...
            success = await self.db.enqueue_task(
                task_id=task_id,
    ...
    async def test_processor_task_scanning_efficiency(self, mock_sleep):
        ...
        processor = await self.create_mock_processor()
        ...
        queued_tasks = await self.queue_test_tasks(task_configs)
        ...
        self.assertLess(
            avg_scan_time,
            0.01,
            f"Average scan time {avg_scan_time:.4f}s should be under 10ms",
        )
    ...
    async def test_processor_throughput_under_load(self):
        ...
        for i in range(0, len(queued_tasks), batch_size):
            batch_tasks = (await self.db.get_pending_tasks())[:batch_size]
        ...
        self.assertGreater(
            throughput, 20, f"Throughput {throughput:.1f} tasks/sec should be over 20"
        )
        self.assertLess(
            avg_task_time,
            0.1,
            f"Average task time {avg_task_time:.3f}s should be under 100ms",
        )
        ...
        self.assertEqual(
            ...
            num_tasks,
            f"Should process all {num_tasks} tasks",
        )
    ...
    async def test_task_error_handling_efficiency(self):
        ...
        self.assertEqual(
            len(self.processed_tasks), 10, "All successful tasks should complete"
        )
        ...
        self.assertLess(
            total_time,
            0.5,
            f"Processing with errors took {total_time:.3f}s, should be under 500ms",
        )

        # Verify failed tasks have correct status
        # Note: Task status tracking would need to be implemented separately
        # as get_task method doesn't exist in current AgentDatabase
        self.assertEqual(len(failing_tasks), 5, "Should have 5 failing tasks")
    ...
    async def test_memory_efficiency_under_load(self):
        ...
        for round_num in range(5):
            ...
            task_configs = [
                {
                    "function_name": "mock_fast_task",
                    "entity_id": f"round_{round_num}_entity_{i}",
                }
                for i in range(10)
            ]
            queued_tasks = await self.queue_test_tasks(task_configs)
        ...
        self.assertLess(
            time_ratio,
            2.0,
            f"Performance degraded {time_ratio:.2f}x from first to last round",
        )
        ...
        self.assertEqual(
            len(self.processed_tasks),
            expected_total,
            f"Should process {expected_total} total tasks",
        )
    ...
    async def test_task_queue_polling_efficiency(self):
        ...
        self.assertLess(
            avg_query_time,
            0.005,
            f"Average query time {avg_query_time:.4f}s should be under 5ms",
        )
    ...
    async def test_concurrent_entity_processing(self):
        ...
        for entity_id, configs in entity_configs.items():
            all_tasks.extend(await self.queue_test_tasks(configs))
        ...
        self.assertLess(
            total_time,
            0.3,
            f"Multi-entity processing took {total_time:.3f}s, should benefit from concurrency",
        )
        ...
        for i, task in enumerate(all_tasks):
            ...
            processed_task = next(
                (p for p in self.processed_tasks if p["task_id"] == task["task_id"]),
                None,
            )
            self.assertIsNotNone(
                processed_task, f"Task {task['task_id']} should be processed"
            )
            entity_ids_processed.add(task["entity_id"])

        self.assertEqual(
            len(entity_ids_processed),
            3,
            "Tasks from all 3 entities should be processed",
        )
    ...
    async def test_processor_resource_cleanup(self):
        ...
        self.assertEqual(
            cleanup_count, 3, "All 3 tasks should have cleaned up resources"
        )
