...
from src.agent.tasks.planner_tasks import execute_initial_planning, execute_task_creation, InitialExecutionPlan
...
class LightweightAsyncFlowsTestCase(unittest.IsolatedAsyncioTestCase, AsyncWarningCaptureMixin):
    ...
    async def lightweight_mock_context(self):
        ...
        with patch('src.agent.tasks.planner_tasks.llm', mock_llm), \
             ...
             patch('src.agent.tasks.file_manager.save_variable_to_file', mock_file_operations), \
             patch('src.agent.tasks.file_manager.save_image_to_file', mock_file_operations):
            
            yield {
                'mock_llm': mock_llm,
                'mock_file_ops': mock_file_operations
            }
        ...
        async with self.capture_async_warnings() as warnings_list:
            async with self.lightweight_mock_context() as mocks:
                ...
                mocks['mock_llm'].a_get_response.side_effect = [
                    ...
                    type('MockResponse', (), {'content': "# Integration Test Response"})()
                ]
                ...
                self.assertLessEqual(performance['execution_time'], 
                ...
                self.assertIsNotNone(planner_data, "Planner should be created in database")
                self.assertIn(planner_data['status'], ['active', 'executing', 'planning'], 
...
class TestDatabaseAsyncOperations(LightweightAsyncFlowsTestCase):
    ...
    async def test_concurrent_database_async_operations(self):
        ...
        async with self.capture_async_warnings() as warnings_list:
            ...
            self.assertEqual(len(all_planners), 3, 
    ...
    async def _execute_concurrent_database_operations(self):
        ...
        for planner_id, _ in planner_tasks:
            update_task = self.db.update_planner(
                planner_id, 
                status="executing",
                objective="Concurrent execution test"
            )
...
class TestCriticalPathAsyncValidation(LightweightAsyncFlowsTestCase):
    ...
    async def test_router_agent_coordination_async_flow(self):
        ...
        async with self.capture_async_warnings() as warnings_list:
            ...
            performance = await self.measure_async_performance(
                self._test_router_coordination,
                router_agent
            )
    ...
    async def _test_router_coordination(self, router_agent):
        ...
        await self.db.update_planner(
            self.planner_id,
            status="executing",
            current_task="integration_test_task"
        )
