...
def get_text_and_table_json_from_image(image: Union[Image.Image, str]) -> str:
    ...
    llm = AzureLLM()  # Initialize Azure OpenAI LLM service
    ...
    full_md = llm.get_response(
        messages=[image_message], model="gpt-4.1-if-global"
    ).content
    ...
    final_md = llm.get_response(messages=messages, model="gpt-4.1-if-global").content
...
def get_chart_readings_from_image(image: Union[Image.Image, str]) -> str:
    ...
    response = llm.get_response(
        ...
        model="gpt-4.1-if-global",  # Using the vision-capable model for image processing
...
def get_img_breakdown(base64_image: str):
    ...
    from agent.models import ImageBreakdown
...
def search_doc(question: str, criteria, doc):
    ...
    # response = llm.get_response(
    #     messages=messages + extension_messages, model="gpt-4.1-if-global"
    # ).content
    # return response
